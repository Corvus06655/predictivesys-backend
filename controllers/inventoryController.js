    ...
    for await (const row of stream) {
      ...
      try {
        const item = {
          ...
          sellingPrice: parseFloat(row.sellingprice || '0') || 0,
        ...
        if (!item.name || !item.sku || !item.category) {
          errors.push({ rowNumber: rowIndex, row, error: 'Missing required fields: name, sku, category' });
          continue;
        }
        ...
        errors.push({ rowNumber: rowIndex, row, error: rowError.message });
      }
    }
