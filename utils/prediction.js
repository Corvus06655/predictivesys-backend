...
const fallbackConsumption = (product) => {
  const quantity = product.quantity || 0;
  const reorderQuantity = product.reorderQuantity || 0;

  if (reorderQuantity > 0) {
    return Math.max(1, reorderQuantity / 90);
  }
...
const generatePrediction = (product, history) => {
  ...
  const effectiveConsumption = dailyConsumption > 0 ? dailyConsumption : fallbackConsumption(product);
  const daysUntilStockout = predictStockout(product.quantity, effectiveConsumption);
  ...
  const forecast = {
    next30Days: Math.ceil(effectiveConsumption * 30),
    next60Days: Math.ceil(effectiveConsumption * 60),
    next90Days: Math.ceil(effectiveConsumption * 90),
  };
  ...
  return {
    ...
    dailyConsumption: parseFloat(effectiveConsumption.toFixed(2)),
    daysUntilStockout,
    reorderDate,
    reorderUrgency,
    suggestedOrderQty: Math.max(suggestedOrderQty, product.reorderQuantity || 0),
    forecast,
